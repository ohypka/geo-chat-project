...
class DoctorsProvider(BaseProvider):
    ...
    def fetch(self, location: Location, **options) -> Dict[str, Any]:
        ...
        for item in data.get("data", [])[:limit]:
            ...
            statistics = attr.get("statistics") or {}
            stats = statistics.get("provider-data", {}) if isinstance(statistics, dict) else {}
            ...
            results.append({
                ...
                "waiting_days": stats.get("average-period", 0) if isinstance(stats, dict) else 0,
                "awaiting": stats.get("awaiting", 0) if isinstance(stats, dict) else 0,
                "queue_date": dates.get("date", "") if isinstance(dates, dict) else "",
                "date_updated": stats.get("update") if isinstance(stats, dict) else None,
            })
