"""
Doctors availability data fetching and normalization.

This module provides functions to fetch medical queue data from the
Narodowy Fundusz Zdrowia (NFZ) API and normalize it to a unified format.
"""
import requests
...
def get_location_from_coords(lat: float, lon: float) -> Dict[str, str]:
    ...
    Returns:
        Dictionary with 'city', 'province', and 'province_code'
        
    Raises:
        ValueError: If city or province cannot be determined
...
def get_coordinates_from_address(address: str) -> Dict[str, float]:
    ...
    Returns:
        Dictionary with 'lat' and 'lon' (None if geocoding fails)
    """
    if not address:
...
def get_doctor_availability(
    ...
    Args:
        lat: Latitude
        lon: Longitude
        service_name: Medical service name (e.g., "kardiolog")
        urgent: Whether to search for urgent cases (pilny) or stable (stabilny)
        
    Returns:
        Dictionary with:
        - query: dict with search parameters
        - results: list of up to 10 facilities with availability data
...
def get_doctor_coordinates(
    ...
    Returns:
        Dictionary with:
        - query: dict with search parameters
        - results: list of facilities with coordinates and queue dates
    """
